...
def main() -> int:
    ...
    a(f"- 인증 실호출: **{sv.get('calls', 0)}회**"
      f" (그 밖에 미인증 생존확인 {_un}회)")
    a(f"- 공식 소스 확보: **{prog.get('completedDays', 0)}일** "
      f"({prog.get('partialWindowStart') or '-'} ~) · "
      f"실패 {prog.get('failedDays', 0)}일 · "
      f"R27 필요 {prog.get('requiredDaysFullWindow')}일 대비 "
      f"**{cov.get('dayCoveragePct')}%**")
    ...
    elif sv.get("verdict") == "PROBE_PASS_PARTIAL_PERIOD_ONLY":
        hstart = (cov.get("sourceHistoryStart") or "?")
        a("> 지난 라운드의 \"인증이 안 먹는다\" 는 **오진이었다.** 원인은 인증도")
        a("> 페이지 크기도 아니고 요청에 붙인 **User-Agent 헤더** 하나였다 —")
        a("> 그걸 빼자 같은 키로 즉시 `NORMAL SERVICE` 가 돌아왔다.")
        a("> ")
        a("> 고치고 나니 이 소스는 거의 모든 축에서 합격이다. 거래량·거래대금")
        a("> 둘 다 공식 필드로 오고, 상폐종목 과거 이력도 나오고, 우선주도")
        a("> 정상이고, KOSPI·KOSDAQ 다 있다. **딱 하나가 걸린다 —**")
        a(f"> **이 소스에는 {hstart} 이전이 없다.** 이분탐색으로 경계를 확정했다.")
        a("> ")
        a("> 그래서 받을 수 있는 구간은 **전부 받았고**(1,552일, 실패 0),")
        a("> R27 은 열지 않았다. 연도 수가 기준에 못 미치기 때문이다. 기준을")
        a("> 낮추면 열리지만 그건 연구를 망치는 짓이라 하지 않았다.")
        a("> ")
        a("> 다만 **길은 있다.** KRX 공식 Open API(2010~)를 붙이면 17개 연도가")
        a("> 되어 기준을 넘는다. 2007~2009 를 영영 못 채워도 그렇다.")
    else:
    ...
    # ── 5. 전체수집 ──────────────────────────────────────────
    # ── 4-5. 2026-08-23 근본원인 ─────────────────────────────
    a("### 4-5. ★ 근본원인 교정 — 왜 전부 API_ERROR:10 이었나")
    a("")
    a("직전 라운드는 credential 이 PRESENT 인데도 모든 호출이")
    a("`INVALID_REQUEST_PARAMETER_ERROR`(코드 10) 로 실패했다. 1차 가설은")
    a("`numOfRows=10000` 이 페이지 크기 상한을 넘었다는 것이었다.")
    a("")
    a("**그 가설은 틀렸다.** 같은 키로 `numOfRows=10` 을 보내도 똑같이 실패했다.")
    a("")
    a("한 번에 한 변수만 바꿔 대조한 결과 원인은 **User-Agent 헤더**였다.")
    a("")
    a("```")
    a("동일 요청(basDt=20260803, numOfRows=10, 같은 serviceKey) 3종 대조")
    a("  UA 없음                                   HTTP 200  NORMAL SERVICE")
    a("  UA 'Mozilla/5.0 (research; contact=wababa)'  HTTP 400  INVALID_REQUEST_PARAMETER_ERROR")
    a("  UA 'python-requests/2'                    HTTP 200  NORMAL SERVICE")
    a("```")
    a("")
    a("게이트웨이가 그 UA 문자열을 거부한다. 파라미터도 인증키도 멀쩡했다.")
    a("이 UA 는 R29 probe 에서 그대로 물려받은 것이다 — 즉 R29 가 \"미인증이라")
    a("코드 10\" 이라고 읽은 응답도 실은 같은 원인이었을 수 있다.")
    a("")
    a("**교정**")
    a("")
    a("```")
    a("① UA               커스텀 UA 제거 → requests 기본 UA 사용 (근본원인)")
    a("② quota / page     DAILY_CALL_QUOTA 와 MAX_ROWS_PER_PAGE 를 분리")
    a("                   (뜻이 다른데 같은 값으로 묶여 함께 움직였다)")
    a("③ adaptive         페이지 크기 오류 시 10000→5000→2000→1000→500→100")
    a("                   으로 낮춰 재시도하고, 낮아진 값을 기억한다")
    a("④ 코드 10 해석      파라미터 오류로만 취급. 인증 사유로 분류하지 않는다")
    a("⑤ credential       Windows 사용자 환경변수도 읽는다 — setx 직후")
    a("                   재시작 안 한 터미널에서 ABSENT 오판을 막는다")
    a("```")
    a("")
    a("실측 페이지 크기 — 상한은 문제가 아니었다.")
    a("")
    a("```")
    a("numOfRows   10 / 100 / 1000 / 2000 / 3000 / 5000 / 10000   전부 OK")
    a("2026-08-03 전종목 2,872행이 10000 한 콜에 전부 들어왔다")
    a("→ 하루치 = 1콜. 4,640일도 일일 10,000 호출 안에 들어간다")
    a("```")
    a("")

    # ── 4-6. 실측 시작일 ─────────────────────────────────────
    a("### 4-6. ★ 소스 historical 시작일 (실측)")
    a("")
    a("\"2007 이 안 나온다\" 로 끝내면 어디부터 되는지를 모른다. 거래일")
    a("캘린더 위에서 이분탐색으로 경계를 확정했다.")
    a("")
    a("```")
    a(f"historyStartDate       {hs.get('historyStartDate')}")
    a(f"직전 거래일(데이터 없음)   {hs.get('lastDateWithoutData')}")
    a(f"연속성 검증              {hs.get('verified')}  (직후 2거래일 추가 확인)")
    a(f"status                 {hs.get('status')}")
    a(f"소요 호출               {hs.get('calls')}회")
    a("```")
    a("")
    a("**이것이 R30 의 결론을 가른 사실이다.** 인증도 되고 필드도 다 있고")
    a("상폐이력도 우선주도 정상인데, 이 소스에는 **2020-01-02 이전이 없다.**")
    a("")

    # ── 4-7. stitching 산술 ──────────────────────────────────
    a("### 4-7. R27 coverage gate 산술 (threshold 변경 0)")
    a("")
    a("R27 precommit 은 연도별 90% coverage 를 만족하는 해가 **최소 15개**")
    a("여야 한다고 고정해 뒀다. 이 기준을 건드리지 않고 산술만 한다.")
    a("")
    stitch = cov.get("stitching") or {}
    a("```")
    a(f"R27 필요 거래일   {cov.get('requiredDays')}일 "
      f"/ 최소 {stitch.get('minYearsRequired')}개 연도 필요")
    a("")
    for o in (stitch.get("options") or []):
        a(f"{o['startDate']} 이후  {o['days']:>5}일 / {o['years']:>2}개 연도"
          f"  -> {'충족' if o['meetsMinYears'] else '미달'}   {o['label']}")
        a(f"{'':>13}  근거: {o['basis']}")
    a("```")
    a("")
    a("**단독으로는 못 넘고, KRX 공식 Open API 를 붙이면 넘는다.** 2007~2009 를")
    a("영영 못 채워도 15개 연도 기준은 충족된다 — 기준을 낮춰서가 아니라")
    a("원래 그렇게 쓰여 있어서다.")
    a("")
    ...
    a(f"수집 모드            {prog.get('acquisitionMode')}")
    a(f"부분수집 시작일        {prog.get('partialWindowStart')}")
    a(f"required (전체 창)    {prog.get('requiredDaysFullWindow')}")
    a(f"required (수집 창)    {prog.get('requiredDays')}")
    a(f"R27 상속 일수         {prog.get('inheritedR27Days')}")
    a(f"누적 확보(수집 창)      {prog.get('completedDays')} / "
      f"{prog.get('requiredDays')}  ({prog.get('dayCoveragePct')}%)")
    a(f"이번 실행 신규 취득      {prog.get('newlyAcquiredDays', 0)}"
      f"   (0 = 이미 캐시에 있어 재수집하지 않음 = resume 정상)")
    ...
    # ── 11. HOTG ─────────────────────────────────────────────
    a("## 10-B. 인증키 rotation (§15)")
    a("")
    a("```")
    a("status                 ROTATION_REQUIRED_AFTER_R30")
    a("이유                    이 키가 R30 이전 인수인계 과정에서 외부 채팅")
    a("                       표면에 노출된 이력이 있다")
    a("이번 세션의 노출          0  (값 출력·보고서 기록·commit 전부 0)")
    a("Claude 가 실행한 조치     없음 — 재발급·폐기·환경변수 변경은 §15 승인 게이트")
    a("권장 시점                R27 판정이 끝난 뒤 (지금 바꾸면 수집이 끊긴다)")
    a("권장 절차                data.go.kr 마이페이지에서 인증키 재발급 →")
    a("                       setx 로 재등록 → 이전 키 폐기")
    a("```")
    a("")
    a("지금 당장 바꾸라는 뜻이 아니다. 이 키는 무료 공개데이터 조회 전용이고")
    a("쓰기 권한도 결제 수단도 붙어 있지 않아 긴급도는 낮다. 다만 노출 이력이")
    a("있으니 R27 이 끝나면 정리하는 편이 맞다.")
    a("")
    ...
    a("")
    if sv.get("verdict") == "PROBE_PASS_PARTIAL_PERIOD_ONLY":
        a("- **Founder 행동: 1건.**")
        a("")
        a("```")
        a("KRX 공식 Open API 인증키(AUTH_KEY) 발급 신청")
        a("")
        a("  왜 이것 하나인가")
        a("    · 공공데이터포털은 2020-01-02 부터만 준다 (실측 확정)")
        a("    · R27 은 연도별 90% coverage 를 만족하는 해가 15개 이상 필요")
        a("    · 2020+ 만으로는 7개 연도 → 미달")
        a("    · KRX Open API(문서상 2010-01-04~) 를 붙이면 17개 연도 → 충족")
        a("    · 즉 threshold 를 건드리지 않고 R27 을 끝낼 수 있는 유일한 경로")
        a("")
        a("  절차")
        a("    1) openapi.krx.co.kr 가입 → 유가증권/코스닥 일별매매정보 활용신청")
        a("    2) 승인 후 발급된 AUTH_KEY 를 등록")
        a("         setx KRX_OPENAPI_AUTH_KEY \"<발급 키>\"")
        a("       → 새 터미널을 열어야 반영된다")
        a("")
        a("  · 무료다. 다만 공공데이터포털과 달리 승인 절차가 있다.")
        a("  · env 는 제가 건드리지 않습니다. 대장이 직접 넣습니다(§15 승인 게이트).")
        a("  · R28 에서 금지된 KRX **웹 스크래핑**과는 다른 것이다 —")
        a("    이건 KRX 가 자동화용으로 제공하는 공식 API 다.")
        a("```")
        a("")
        a("- 다음 작업 결정 규칙: **D** (§33 — liquidity gap 하나만 해결)")
        a("")
        a("- 다음 단일 작업: **KRX Open API AUTH_KEY 를 받으면 2010~2019 구간을 "
          "그 소스로 받아 이미 확보한 2020~2026 공공데이터포털 구간과 stitching "
          "하고, 두 소스 겹치는 구간에서 교차검증한 뒤 R27 coverage gate 를 "
          "재측정한다. 새 factor 연구·threshold 변경 금지.**")
    elif sv.get("verdict") == "CREDENTIAL_NOT_EFFECTIVE":
        a("- **Founder 행동: 1건.**")
        a("")
        a("```")
