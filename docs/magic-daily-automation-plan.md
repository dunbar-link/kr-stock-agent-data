...
- **다음 = 45-AUTO3**: B 티어(approval-ticket) — magic_make_approval_ticket.py(receipt+PENDING ticket) →
  magic_apply_from_approval.py(승인 시 apply --confirm) → magic_publish_public.py(승인 시 REPO1 3키).
  A 티어 며칠 무인 관찰 후 착수. C 티어(실주문/commit/push/deploy)는 영구 사람 승인.
